...
void save_state_to_flash(uint32_t duty, int days, int hours, int mins, int secs) {
    ...
    memcpy(page_buffer, &state, sizeof(struct nv_recovery_t));
...
bool load_state_from_flash(struct nv_recovery_t *state_out) {
    ...
    const struct nv_recovery_t *flash_data = (const struct nv_recovery_t *)(XIP_BASE + FLASH_TARGET_OFFSET);
    
    // Direct memory copy via pointer mapping 
    memcpy(state_out, flash_data, sizeof(struct nv_recovery_t));
